...
namespace OrderMatcher.Types.Serializers
{
    public class BookSerializer : Serializer
    {
        ...
        public static void Serialize(BookDepth book, Span<byte> bytes)
        {
            ...
            Write(bytes[timeStampOffset..], book.TimeStamp);
            Price.WriteBytes(bytes[ltpOffset..], book.LTP ?? 0);
            Write(bytes[bidCountOffset..], (short)book.Bid.Count);
            Write(bytes[askCountOffset..], (short)book.Ask.Count);

            int i = 0;
            foreach (var level in book.Bid)
            {
                var start = bidStartOffset + (i * sizeOfLevel);
                Price.WriteBytes(bytes[start..], level.Key);
                Quantity.WriteBytes(bytes[(start + sizeOfPrice)..], level.Value);
                if (++i == book.Bid.Count)
                {
                    break;
                }
        ...
        public static BookDepth Deserialize(ReadOnlySpan<byte> bytes)
        {
            ...
            var timeStamp = BitConverter.ToInt32(bytes[timeStampOffset..]);
            Price? ltp = Price.ReadPrice(bytes[ltpOffset..]);
            if (ltp == 0)
            {
                ltp = null;
            }
            ...
            {
                var price = Price.ReadPrice(bytes[(bidStartOffset + (i * sizeOfLevel))..]);
                var quantity = Quantity.ReadQuantity(bytes[(bidStartOffset + (i * sizeOfLevel) + sizeOfPrice)..]);
                bid.Add(price, quantity);
            }
            var askStartOffset = bidStartOffset + (bidCount * sizeOfLevel);
            for (int i = 0; i < askCount; i++)
            {
                var price = Price.ReadPrice(bytes[(askStartOffset + (i * sizeOfLevel))..]);
                var quantity = Quantity.ReadQuantity(bytes[(askStartOffset + (i * sizeOfLevel) + sizeOfPrice)..]);
